...
app.config.update(
    ...
    MAIL_PASSWORD='kukeymkzlkqzjupm'
)
...
@app.route("/")
def home():
    # flash("Explore our blog","success")
    posts = Posts.query.filter_by().all()
    last = math.ceil(len(posts) / int(params['no_of_posts']))
    page = request.args.get('page')

    # Default to page 1 if page is not numeric or not provided
    ...
    # Calculate the start and end indices for slicing posts
    start = (page - 1) * int(params['no_of_posts'])
    end = start + int(params['no_of_posts'])
    posts = posts[start:end]

    # Set up the Previous and Next URLs
    prev = "#"
    next = "#"
    if page > 1:
        prev = f"/?page={page - 1}"
...
def contact():
    if(request.method=='POST'):
        ...
        '''Send email notification to the Gmail address'''
        mail.send_message(
